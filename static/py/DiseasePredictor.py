...
class DiseasePredictor:
    def __init__(self):
        self.model = pickle.load(open("static/models/svc.pkl", "rb"))

        self.diets = pd.read_csv("static/datasets/diets.csv")
        self.workout = pd.read_csv("static/datasets/workout_df.csv")
        self.remedies = pd.read_csv("static/datasets/remedies.csv")
        self.symptoms = pd.read_csv("static/datasets/symtoms_df.csv")
        self.precautions = pd.read_csv("static/datasets/precautions_df.csv")
        self.description = pd.read_csv("static/datasets/description.csv")
        self.review =pd.read_csv("static/datasets/review.csv")
    ...
    def predict_disease(self, patient_symptoms):
        ...
        return self.diseases[self.model.predict([input_vector])[0]]
    


    def get_disease_details(self, predicted_disease):
        disease_description = self.description[self.description['Disease'] == predicted_disease]['Description']
        disease_description = " ".join([word for word in disease_description])

        precautions_info = self.precautions[self.precautions['Disease'] == predicted_disease][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']].values.tolist()[0]
        precautions_list = [item for item in precautions_info if isinstance(item, str)]

        remedies_info = self.remedies[self.remedies['Disease'] == predicted_disease][['Remedy1', 'Remedy2', 'Remedy3']].values.tolist()[0]
        remedies_list = [item for item in remedies_info if isinstance(item, str)]

        diet_info = self.diets[self.diets['Disease'] == predicted_disease]['Diet']
        diet_list = [diet for diet in diet_info.values][0]

        workout_info = self.workout[self.workout['disease'] == predicted_disease]['workout']
        workout_list = [workout for workout in workout_info.values]

        return disease_description, precautions_list, remedies_list, ast.literal_eval(diet_list), workout_list
       


    #     # Function to recommend medicine for given diseases with useful count
    #     def recommend_medicine(self,predicted_dis):
    #         recommended_medicines = {}
    #         for disease in predicted_dis:
    #         # Filter dataset based on the disease
    #             filtered_data = self.review[self.review['Diseases Name'] == disease]
    ...
    #             'Top Medicine': self.top_medicine['Remedi Name '] if top_medicine is not None else None,
    #             'Other Remedies':self.sorted_data['Remedi Name '].tolist()[1:]
    #         }
    #         return recommended_medicine
    # # Recommend medicines for given diseases
    #     recommendations = recommend_medicine(self,predicted_disease)
    ...
    #     print(f"For {disease}:")
    #     print(f"Top Medicine: {medicines['Top Medicine']}")
